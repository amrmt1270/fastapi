...
async function updateMemo(memo){
    ...
    {
        console.error('メモ更新中にエラーが発生しました');
    }
}

async function deleteMemo(memoId){
    try {
        const response = fetch('${apiUrl}${memoId}', 
            {
                method : 'DELETE'
            });
        const data = (await response).json();
...
async function editMemo (memoId) {
    ...
    document.querySelector('#creaeMemoForm button[type="submit"]').style.display = 'none';
}

document.addEventListener('DOMContentLoaded', () => {
    const form = document.getElementById('createMemoForm');

    from.onsubmit = async (event) => {
